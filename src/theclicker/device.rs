...
use input_linux::{
    EvdevHandle, EventKind, EventTime, InputEvent, Key, KeyEvent, KeyState, SynchronizeEvent,
    UInputHandle, sys::input_event,
};

use super::{choose_usize, choose_yes};
...
impl OutputDevice {
    pub fn uinput_open(path: PathBuf) -> Result<Self, String> {
    ...
    pub fn create(&self, from: &InputDevice) {
        let id = from.handler.device_id().unwrap();
        let name = from.handler.device_name().unwrap();
        self.handler
            .create(&id, &name, input_linux::sys::FF_MAX_EFFECTS as u32, &[])
            .unwrap();
    }
